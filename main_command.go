...
var runCommand = cli.Command{
	...
	Action: func(context *cli.Context) error {
		...
		if createTty && detach {
			return fmt.Errorf("参数 ti 和 d 不能同时存在")
		}
		...
		log.Infoln("镜像名：", imageName)
		log.Infoln("容器名：", containerName)
		log.Infoln("命令：", cmdArray)
		log.Infoln("容器卷：", volume)
		log.Infoln("网络：", network)
		log.Infoln("环境变量：", envSlice)
		log.Infoln("端口映射：", portmapping)
		log.Infoln("运行参数设置：", resConf)
